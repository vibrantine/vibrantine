...
- When you have enough to answer, call `conclude` with `answer` (your synthesized answer) and `claims` (the load-bearing assertions). Each claim is an object, never a bare string: `value` (the fact), `source_urls` (the fetched URLs that support it), and `confidence` (one of 'verified', 'grounded', 'speculative'). Example claim: `{"value": "The Eiffel Tower is 330 metres tall.", "source_urls": ["https://en.wikipedia.org/wiki/Eiffel_Tower"], "confidence": "grounded"}`
